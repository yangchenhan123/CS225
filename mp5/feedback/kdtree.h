...
class KDTree
{
  ...
  public:
    ...
    void quick_select(int left, int right, int pivot, int curDim);
    void swap_(vector<Point<Dim>> &vec, int a, int b); //helper function for quick_select
    KDTreeNode* buildTree(int left, int right, int curDim);
     ...
     */
    ~KDTree();
    void delete_tree(KDTree<Dim>::KDTreeNode* curr);
    void delete_();
    /**
    ...
    Point<Dim> findNearestNeighbor_helper(const Point<Dim>& query, typename KDTree<Dim>::KDTreeNode* curr, int curDim) const;
    double dist(const Point<Dim>& p1, const Point<Dim>& p2) const;
    ...
    vector<Point<Dim>> getPoints() const;
