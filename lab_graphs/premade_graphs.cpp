...
Graph PremadeGraphs::createChinaMap(bool isWeighted){
    ...
    size_t numCities = 10;
    string labels[]
        = {"Hangzhou", "Shanghai", "Changsha", "Wuhan",    "Hefei",
           "Dalian",     "Jiangsu",  "Chengdou",   "Chongqing", "Beijing"};
    vector<Vertex> vertices(numCities);
    for (size_t i = 0; i < numCities; ++i) {
        vertices[i] = g.insertVertex(labels[i]);
        g.setVertexName(i, labels[i]);
    ...
    g.insertEdge(vertices[6], vertices[1]);
    g.insertEdge(vertices[5], vertices[7]);
    g.insertEdge(vertices[8], vertices[4]);
    g.insertEdge(vertices[1], vertices[7]);
    g.insertEdge(vertices[8], vertices[2]);
    g.insertEdge(vertices[1], vertices[2]);
    g.insertEdge(vertices[2], vertices[9]);
    ...
    if (isWeighted) {
        g.setEdgeWeight(vertices[4], vertices[6], 456);
        g.setEdgeWeight(vertices[0], vertices[3], 234);
        g.setEdgeWeight(vertices[0], vertices[5], 478);
        g.setEdgeWeight(vertices[0], vertices[6], 263);
        g.setEdgeWeight(vertices[2], vertices[6], 374);
        g.setEdgeWeight(vertices[3], vertices[8], 146);
        g.setEdgeWeight(vertices[3], vertices[1], 567);
        g.setEdgeWeight(vertices[6], vertices[1], 451);
        g.setEdgeWeight(vertices[5], vertices[7], 839);
        g.setEdgeWeight(vertices[8], vertices[4], 286);
        g.setEdgeWeight(vertices[1], vertices[7], 205);
        g.setEdgeWeight(vertices[8], vertices[2], 306);
        g.setEdgeWeight(vertices[1], vertices[2], 681);
        g.setEdgeWeight(vertices[2], vertices[9], 386);
    }
