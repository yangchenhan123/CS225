...
size_t insertion_idx_helper(unsigned front, unsigned end, const C& val ,const std::vector<T>& elements)
{
    /* TODO Your code goes here! */
    if(elements[(front+end)/2] == val){return (front+end)/2;}
    else if(elements[(front+end)/2] < val && elements[(front+end)/2+1] > val){return (front+end)/2+1;}
    else if(elements[(front+end)/2] > val){return insertion_idx_helper(front, (front+end)/2, val, elements);}
    else{return insertion_idx_helper((front+end)/2 + 1, end, val, elements);}

    while(front <= end){
        if(elements[(front+end)/2] == val){return (front + end) / 2;}
        else if(elements[(front+end)/2] > val){end = (front+end)/2;}
        else{front = (front+end)/2 + 1;}
    }
    return end;
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val){
    if(elements.size() == 0 || elements[0] > val){return size_t(0);}
    if(elements[elements.size() - 1] < val){return size_t(elements.size());}
    return insertion_idx_helper(0, elements.size() - 1, val, elements);
