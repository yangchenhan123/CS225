                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    // your code!
    Frequency single = singleQueue.front()->freq;
    Frequency merge = mergeQueue.front()->freq;
    if (single < merge){
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    //put all frequencies into singlequeue
    for(unsigned i = 0; i < frequencies.size(); i++){
        TreeNode* add = new TreeNode(frequencies[i]);
        singleQueue.push(add);
    }
    
    //get the huffman tree
    TreeNode *merge_node1;
    TreeNode *merge_node2;
    while (singleQueue.size() + mergeQueue.size() > 1){
        //find merge_node1
        if(singleQueue.size() != 0 && mergeQueue.size() != 0){
            merge_node1 = removeSmallest(singleQueue, mergeQueue);
        }else if(singleQueue.size() == 0 && mergeQueue.size() != 0){
            merge_node1 = mergeQueue.front();
            mergeQueue.pop();
        }else{
            merge_node1 = singleQueue.front();
            singleQueue.pop();
        }
        
        //find merge_node2
        if(singleQueue.size() != 0 && mergeQueue.size() != 0){
            merge_node2 = removeSmallest(singleQueue, mergeQueue);
        }else if(singleQueue.size() == 0 && mergeQueue.size() != 0){
            merge_node2 = mergeQueue.front();
            mergeQueue.pop();
        }else{
            merge_node2 = singleQueue.front();
            singleQueue.pop();
        }

        //build new node and push it to mergeQueue
        int new_freq = merge_node1->freq.getFrequency() + merge_node2->freq.getFrequency();
        TreeNode *mergered = new TreeNode(new_freq);
        if(merge_node1->freq < merge_node2->freq){ // "<" here!!!
            mergered->left = merge_node1;
            mergered->right = merge_node2;
        }else{
            mergered->left = merge_node2;
            mergered->right = merge_node1;
        }
        mergeQueue.push(mergered);
    }
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        while (current->left != nullptr && current->right != nullptr){
            ...
            current = (!next_bit) ? current->left : current->right;
        }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (!bfile.hasBits())
        ...
        temp = new TreeNode(new_frequency);
    }else{
        temp = new TreeNode(0);
