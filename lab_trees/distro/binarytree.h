...
class BinaryTree
{
  ...
  public:
     ...
     */
    bool isOrdered(Node* subRoot, Node* temp_left, Node* temp_right) const;
     ...
     */
    void printPaths(vector<vector<T> > &paths, Node* subRoot, vector<T> temp) const;
     ...
     * helper function for printPaths()
     */
    int sumDistances(Node* subRoot, int& sum, int curr) const;
